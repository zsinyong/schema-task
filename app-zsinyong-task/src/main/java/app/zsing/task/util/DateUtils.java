...
public class DateUtils {
    ...
    public static long getUnixtimestamp() {
        long time = 0;
        try {
            Date date = new SimpleDateFormat(DATETIME_FORMAT)
                    .parse(getDateTime());
     ...
     * @return
     */
    public static long getUnixtimestamp(String date, String pattern) {
        long time = 0;
        try {
            String[] times;
            if (TIME_FORMAT.equalsIgnoreCase(pattern)) {
                times = date.split(":");
                //时
                int hour = Integer.parseInt(times[0]);
                //分
                int minte = Integer.parseInt(times[1]);
                //秒
                int seconds = (int) Double.parseDouble(times[2]);
                time = hour * 60 * 60 + minte * 60 + seconds;
            } else {
    ...
    public static String getDateTime(Date date) {
        return formatDate(date, DateUtils.DATE_FORMAT);
    }
     ...
     * @since 1.0
     */
    public static String getDateTimeNoSeparator() {
        return formatDate(new Date(), DateUtils.DATETIME_FORMAT_NO_SEPARATOR);
    }
